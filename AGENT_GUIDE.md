...
系统支持**多 Agent 隔离**，每个 Agent 有独立的：
- 独立的 session 目录：`/root/.openclaw/agents/{agent}/sessions/`
- 独立的 Qdrant collection：`mem0_{agent}`
- 独立的 watch 进程和 cron 任务

**示例 Agent**：main、capital、dev、legal 等

每个 Agent 的 watch_sessions.js 进程独立运行，互不影响。
